...
export default function LogoIntro({ onStartExit, onComplete }: LogoIntroProps) {
      ...
      if (isWarping) {
        ...
        for (let i = 0; i < stars.length; i++) {
          ...
          if (px >= 0 && px <= width && py >= 0 && py <= height) {
            ...
            if (streakDist > 8) {
              // Glowing warp streak with cyan-white accents
              const gradient = ctx.createLinearGradient(prevPx, prevPy, px, py);
              gradient.addColorStop(0, `rgba(0, 229, 255, ${alpha * 0.25})`);
              gradient.addColorStop(0.7, `rgba(0, 229, 255, ${alpha * 0.85})`);
        ...
        // Idle gentle twinkling celestial particles in deep space
  ...
  useEffect(() => {
    ...
    const ringConfigs = [
      { radius: 38, count: 20, dotSize: 2.5, color: '0, 229, 255' },
      { radius: 30, count: 16, dotSize: 2.8, color: '0, 180, 255' },
      { radius: 22, count: 12, dotSize: 3.0, color: '0, 130, 230' },
      { radius: 14, count: 8,  dotSize: 3.2, color: '255, 177, 59' },
      { radius: 6,  count: 4,  dotSize: 3.5, color: '255, 210, 100' },
    ];
    ...
    function drawEmblem(t: number) {
      ...
      if (state.revealProgress > 80) {
        ...
        grd.addColorStop(0, `rgba(0, 229, 255, ${0.9 * centerPulse})`);
        grd.addColorStop(0.3, `rgba(0, 180, 255, ${0.4 * centerPulse})`);
    ...
    >
      ...
      <div className="absolute top-6 right-6 z-30 pointer-events-auto">
        <button
          ...
          className="px-3.5 py-1.5 rounded-full text-xs font-mono tracking-wider text-slate-400 hover:text-cyan-300 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-cyan-500/40 backdrop-blur-md transition-all duration-200 cursor-pointer flex items-center gap-1.5 shadow-sm"
        >
