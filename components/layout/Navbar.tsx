...
export const Navbar: React.FC<NavbarProps> = ({ onOpenDemo }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#090d16] border-b border-zinc-800 shadow-lg py-3.5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="flex items-center justify-between min-h-[44px]">
          ...
          <Link
            href="/"
            className="flex items-center outline-none focus:outline-none focus:ring-0 select-none shrink-0"
            ...
            {NAV_ITEMS.map((item) => {
              ...
              if (hasSubItems) {
                return (
                  <div key={item.label} className="relative group/dropdown py-2">
                    <Link
                      href={item.href}
                      className={`relative py-1 text-[13px] xl:text-sm font-medium tracking-wide flex items-center gap-1.5 cursor-pointer select-none ${
                        isActive ? 'text-cyan-400 font-semibold' : 'text-slate-300 hover:text-white'
                      }`}
                    >
                      <span>{item.label}</span>
                      <ChevronDown className="w-3.5 h-3.5 text-slate-400 group-hover/dropdown:text-cyan-400 shrink-0" />
                      {isActive && (
                        <span className="absolute -bottom-1.5 left-0 right-0 h-[2px] bg-cyan-400 rounded-full" />
                      )}
                    </Link>

                    {/* Dropdown Menu (No sliding or fading delay) */}
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-[380px] hidden group-hover/dropdown:block z-50">
                      <div className="h-[2px] w-full bg-cyan-500 rounded-t-sm" />
                      <div className="bg-[#0f1117] border-x border-b border-zinc-800 shadow-2xl rounded-b-xl overflow-hidden p-2 divide-y divide-zinc-800/50">
                            ...
                            <Link
                              ...
                              className={`block p-3 rounded-lg ${
                                isCurrentSub
                                  ...
                                  : 'hover:bg-white/[0.06] border-l-2 border-transparent hover:border-cyan-400'
                              }`}
                            >
                              <div className="flex items-center justify-between mb-1">
                                <span className={`text-xs font-semibold tracking-wide uppercase ${
                                  isCurrentSub ? 'text-cyan-300' : 'text-slate-200'
                                }`}>
                                  {sub.label}
                                </span>
                                {sub.badge && (
                                  <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-cyan-500/20 border border-cyan-400/40 text-cyan-300">
                                    {sub.badge}
                                  </span>
                                )}
                              </div>
              ...
              return (
                <Link
                  key={item.label}
                  href={item.href}
                  className={`relative py-1 text-[13px] xl:text-sm font-medium tracking-wide ${
                    isActive ? 'text-cyan-400 font-semibold' : 'text-slate-300 hover:text-white'
                  }`}
                >
                  <span>{item.label}</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs xl:text-sm font-semibold tracking-wide text-black bg-cyan-400 hover:bg-cyan-300 cursor-pointer"
            >
            ...
            <button
              ...
              className="p-2 rounded-xl bg-zinc-900 border border-white/10 text-slate-300 hover:text-white focus:outline-none"
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden px-4 pt-3 pb-8 space-y-4 bg-[#090d16] border-b border-zinc-800 mt-3 shadow-2xl">
          {/* Mobile Booking Header */}
          <div className="p-3.5 rounded-xl bg-slate-900 border border-zinc-800 flex items-center justify-between">
            <div>
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-cyan-400 hover:bg-cyan-300 text-black text-xs font-bold tracking-wide"
            >
              Book Call
            </button>
          </div>
              ...
              if (hasSubItems) {
                return (
                  <div key={item.label} className="flex flex-col">
                    ...
                    {isDropdownOpen && (
                      <div className="p-2 bg-zinc-900 rounded-lg mt-1 border border-zinc-800 space-y-1">
                        {item.subItems!.map((sub) => (
                          <Link
                            key={sub.label}
                            href={sub.href}
