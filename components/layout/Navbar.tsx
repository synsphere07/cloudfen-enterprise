  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-[#080a0c]/90 backdrop-blur-xl border-b border-amber-500/20 shadow-[0_4px_30px_rgba(0,0,0,0.85)] py-3'
    ...
    >
                ...
                return (
                  <div key={item.label} className="relative group/dropdown py-2">
                    ...
                    >
                      ...
                      <ChevronDown className="w-3.5 h-3.5 text-slate-400 group-hover/dropdown:text-amber-400 group-hover/dropdown:rotate-180 transition-transform duration-200 shrink-0" />

                      {/* Active / Hover Line */}
                      {isActive ? (
                        <span className="absolute -bottom-1.5 left-0 right-0 h-[2px] bg-amber-400 rounded-full shadow-[0_0_8px_rgba(255,177,59,0.9)]" />
                      ) : (
                        <span className="absolute -bottom-1.5 left-1/2 right-1/2 h-[2px] bg-transparent rounded-full group-hover/dropdown:left-0 group-hover/dropdown:right-0 group-hover/dropdown:bg-amber-400/60 transition-all duration-200" />
                      )}
                    </Link>
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-[380px] opacity-0 translate-y-2 pointer-events-none group-hover/dropdown:opacity-100 group-hover/dropdown:translate-y-0 group-hover/dropdown:pointer-events-auto transition-all duration-200 ease-out z-50">
                      <div className="h-[2px] w-full bg-gradient-to-r from-amber-500 via-amber-400 to-yellow-500 rounded-t-sm shadow-[0_0_10px_rgba(255,177,59,0.6)]" />
                      <div className="bg-[#111417] border-x border-b border-zinc-800 shadow-[0_20px_50px_rgba(0,0,0,0.9)] rounded-b-xl overflow-hidden p-2 divide-y divide-zinc-800/50">
                            ...
                            <Link
                              ...
                              className={`group/sub block p-3 rounded-lg transition-all duration-150 ${
                                isCurrentSub
                                  ...
                                  : 'hover:bg-white/[0.04] border-l-2 border-transparent hover:border-amber-400'
                              }`}
                            >
                              <div className="flex items-center justify-between mb-1">
                                <span className={`text-xs font-semibold tracking-wide uppercase transition-colors ${
                                  isCurrentSub ? 'text-amber-300' : 'text-slate-200 group-hover/sub:text-white'
                                }`}>
                                  {sub.label}
                                </span>
                                {sub.badge && (
                        ...
                        <div className="pt-2 pb-1 px-2 flex items-center justify-between text-[11px]">
                          ...
                          <Link href="/readiness-sprint" className="text-amber-400 font-semibold hover:underline flex items-center gap-1">
              ...
              return (
                ...
                >
                  ...
                  {isActive ? (
                    ...
                    <span className="absolute -bottom-1.5 left-1/2 right-1/2 h-[2px] bg-transparent rounded-full group-hover:left-0 group-hover:right-0 group-hover:bg-amber-400/60 transition-all duration-200" />
                  )}
                </Link>
              );
            })}
      ...
      >
        <div className="px-4 pt-3 pb-8 space-y-4 bg-[#080a0c]/95 backdrop-blur-2xl border-b border-amber-500/20 mt-3 shadow-2xl">

          {/* Mobile Booking Header */}
          <div className="p-3.5 rounded-xl bg-gradient-to-r from-amber-950/70 via-stone-900 to-amber-950/70 border border-amber-400/30 flex items-center justify-between">
            <div>
              <div className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                <p className="text-[11px] font-bold uppercase text-white tracking-wider">Agent Readiness</p>
              </div>
              <p className="text-[11px] text-amber-300 font-mono mt-0.5">4-Week Production Deploy</p>
            </div>
            <button
              onClick={() => {
              ...
              className="px-3 py-1.5 rounded-lg bg-amber-400 hover:bg-amber-300 text-black text-xs font-bold tracking-wide transition-colors"
            >
              Book Call
            </button>
          </div>
          ...
          <div className="space-y-1">
            {NAV_ITEMS.map((item) => {
                      ...
                      <div className="p-2 bg-zinc-900/70 rounded-lg mt-1 border border-zinc-800 space-y-1">
                        {item.subItems!.map((sub) => (
                          <Link
                            ...
                            className="w-full text-left px-3 py-2 text-xs font-medium text-slate-300 hover:text-amber-300 hover:bg-white/5 rounded-md flex items-center justify-between"
                          >
                            <span className="truncate">{sub.label}</span>
                            {sub.badge && (
                              <span className="text-[9px] font-mono text-amber-400 px-1 py-0.5 rounded bg-amber-500/10">
                                {sub.badge}
                              </span>
                            )}
                          </Link>
              ...
              return (
                ...
                >
                  <span>{item.label}</span>
                  {isActive && <span className="w-1.5 h-1.5 rounded-full bg-amber-400 shadow-[0_0_6px_rgba(255,177,59,0.8)]" />}
                </Link>
              );
            })}
          </div>
